...
export class AffiliateService {
  ...
  async generateAffiliateLink(userId: string, courseId: string): Promise<{ code: string; link: string }> {
    ...
    let existing = user.affiliateLinks.find(l => l.courseId.toString() === courseId);
    if (existing) {
      return { code: existing.code, link: `${process.env.FRONTEND_URL}/aff/${userId}/${courseId}/${existing.code}` };
    }
    let code: string, isUnique = false;
    do {
      code = crypto.randomBytes(6).toString('hex').toUpperCase();
      const existingCode = await User.findOne({ 'affiliateLinks.code': code });
      if (!existingCode) isUnique = true;
    } while (!isUnique);
    user.affiliateLinks.push({ courseId: new mongoose.Types.ObjectId(courseId), code, clicks: 0, conversions: 0, totalEarned: 0, createdAt: new Date() });
    await user.save();
    return { code, link: `${process.env.FRONTEND_URL}/aff/${userId}/${courseId}/${code}` };
  }
  ...
  async trackClick(affiliateUserId: string, courseId: string, code: string, req: any): Promise<void> {
    ...
    logger.info(`Affiliate click tracked: ${affiliateUserId} -> ${courseId}`);
  }
  ...
  async processAffiliateConversion(buyerId: string, courseId: string, transactionId: mongoose.Types.ObjectId, session: mongoose.ClientSession): Promise<void> {
    const click = await AffiliateClick.findOne({ affiliateUserId: { $ne: buyerId }, courseId, converted: false }).sort({ clickedAt: -1 }).session(session);
    if (!click) return;
    click.converted = true; click.conversionAt = new Date(); click.transactionId = transactionId;
    ...
    const commissionTx = new Transaction({
      user: affiliate._id, type: 'commission', subtype: 'affiliate', amount: commission, currency: course.currency,
      status: 'completed', description: `Affiliate commission for "${course.title}"`,
      ...
      fromUserId: buyerId, toUserId: affiliate._id, completedAt: new Date()
    });
    await commissionTx.save({ session });
    logger.info(`Affiliate commission: ₦${commission} to user ${affiliate._id}`);
  }

  async processReferralSignup(referralCode: string, newUserId: string, session?: mongoose.ClientSession): Promise<void> {
    const referrer = await User.findOne({ referralCode });
    if (!referrer) return;
    let level = 1, current = referrer;
    while (current.referredBy && level < 3) { level++; current = await User.findById(current.referredBy); if (!current) break; }
    const referral = new Referral({ referrer: referrer._id, referred: newUserId, level, referralCode, expiresAt: new Date(Date.now() + 30*24*60*60*1000) });
    if (session) await referral.save({ session }); else await referral.save();
  ...
  async processReferralUpgrade(userId: string, amountPaid: number, transactionId: mongoose.Types.ObjectId, session: mongoose.ClientSession): Promise<void> {
    ...
    if (!referral) return;
    referral.status = 'active'; referral.firstPurchaseAt = new Date(); await referral.save({ session });
    ...
    if (referrer && bonusAmount > 0) {
      referrer.walletBalance += bonusAmount; referrer.referralEarnings += bonusAmount; await referrer.save({ session });
      const bonusTx = new Transaction({
        user: referrer._id, type: 'commission', subtype: 'referral', amount: bonusAmount, currency: 'NGN',
        status: 'completed', description: `Referral bonus for user upgrade`,
        ...
        fromUserId: userId, toUserId: referrer._id, completedAt: new Date()
      });
