...
export class UserController {
  ...
  getLeaderboard = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (period === 'week') {
        ...
        matchStage.lastActiveAt = { $gte: weekAgo };
      }
      if (period === 'month') {
        const monthAgo = new Date();
        monthAgo.setMonth(monthAgo.getMonth() - 1);
        matchStage.lastActiveAt = { $gte: monthAgo };
      }
      
      const leaders = await User.find(matchStage)
        .sort({ [sortField]: -1 })
        .limit(Number(limit))
        .select('firstName lastName displayName avatar level xp totalEarned referralEarnings');
  ...
  getDashboardStats = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as any).user?.userId;
      const user = await User.findById(userId)
        .select('xp level streak walletBalance totalEarned')
        .populate('coursesEnrolled', 'title thumbnail progress')
        .populate('certificatesEarned', 'certificateId issueDate');
      ...
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);
      const weeklyXP = await Transaction.aggregate([
        { $match: { user: new mongoose.Types.ObjectId(userId), createdAt: { $gte: weekAgo }, type: 'reward' } },
