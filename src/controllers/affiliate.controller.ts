// ============================================================
// FILE: src/controllers/affiliate.controller.ts (COMPLETE FIXED)
// ============================================================
...
function getFrontendBase(): string {
    return process.env.FRONTEND_URL || process.env.CLIENT_URL || 'https://adc-mu.vercel.app';
}

/**
 * Accept an affiliate offer for a course or book.
...
export const acceptAffiliateOffer = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (bookId) {
            ...
            if (bookLink) {
                const fullLink = `${getFrontendBase()}/books/${bookId}?aff=${bookLink.code}`;
            ...
            const fullLink = `${getFrontendBase()}/books/${bookId}?aff=${bookCode}`;
        ...
        if (link) {
            const fullLink = `${getFrontendBase()}/courses/${courseId}?aff=${link.code}`;
            return res.json({
                success: true,
                data: {
                    ...link.toObject(),
        ...
        const fullLink = `${getFrontendBase()}/courses/${courseId}?aff=${code}`;
    ...
    try {
        ...
        const enriched = links.map(l => {
            ...
            if (book) {
                ...
                return {
                    ...l.toObject(),
                    link: `${getFrontendBase()}/books/${bookId}?aff=${l.code}`,
            ...
            return {
                ...l.toObject(),
                link: `${getFrontendBase()}/courses/${courseId}?aff=${l.code}`,
...
export const trackAffiliateClick = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const redirectUrl = `${getFrontendBase()}${redirectPath}?aff=${code}`;
    ...
    try {
        ...
        const breakdown = await Promise.all(links.map(async (l) => {
            ...
            return {
                ...
                link: `${getFrontendBase()}/courses/${l.courseId}?aff=${l.code}`,
                code: l.code,
            };
        }));
...
/**
 * Get all affiliate offers (courses and books with affiliate enabled).
    ...
    try {
        ...
        const enrichedCourses = courses.map(course => {
            ...
            return {
                ...
                affiliateLink: existingLink ? {
                    ...
                    link: `${getFrontendBase()}/courses/${course._id}?aff=${existingLink.code}`
                } : null,
        ...
        const enrichedBooks = books.map(book => {
            ...
            return {
                ...
                affiliateLink: existingLink ? {
                    code: existingLink.code,
                    link: `${getFrontendBase()}/books/${book._id}?aff=${existingLink.code}`
                } : null,
