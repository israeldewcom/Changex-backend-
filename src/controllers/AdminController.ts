...
export class AdminController {
  ...
  approveCourse = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // ✅ Broadcast affiliate offer to ALL users if course has affiliate commission
      const io = req.app.get('io');
      if (io && course.affiliateCommission && course.affiliateCommission > 0) {
        ...
        // ✅ Create database notifications for ALL users (offline users will see later)
        ...
        for (let i = 0; i < allUsers.length; i += batchSize) {
          ...
          const notifications = batch.map(user => ({
            ...
            data: { courseId: course._id, type: 'affiliate_offer' }
          }));
