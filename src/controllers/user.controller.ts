...
export const uploadAvatar = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = req.user as IUser;
    const result = await uploadToCloudinary(req.file.buffer, 'avatars', { transformation: [{ width: 256, height: 256, crop: 'fill' }] });
...
export const getWallet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({ success: true, data: { balance: user.walletBalance, pending: user.pendingWithdrawal, transactions } });
  } catch (err) { next(err); }
};
...
export const getReferrals = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const formatted = referrals.map(r => ({
      id: r._id,
      name: r.referredId ? `${(r.referredId as any).firstName} ${(r.referredId as any).lastName}` : 'User',
      date: r.createdAt,
      status: r.status,
      earned: r.earned,
    }));
