...
export class CourseController {
  ...
  rateCourse = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const enrollment = await Enrollment.findOne({ user: userId, course: courseId, status: 'completed' });
      if (!enrollment) {
      ...
      if (existingReview) {
        ...
        await Review.create({ user: userId, course: courseId, rating, title: 'Course review', content: review || '', isVerifiedPurchase: true, isApproved: true });
      }
      const allReviews = await Review.find({ course: courseId });
      const avgRating = allReviews.reduce((sum, r) => sum + r.rating, 0) / allReviews.length;
      await Course.findByIdAndUpdate(courseId, { rating: avgRating, reviewCount: allReviews.length });
  ...
  askQuestion = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const newQuestion = new CourseQuestion({ course: courseId, lessonId, user: userId, question });
      ...
      if (course && course.instructor) {
        await this.notificationService.sendNotification(course.instructor._id.toString(), 'course', {
          ...
          metadata: { courseId, questionId: newQuestion._id }
        });
      }
