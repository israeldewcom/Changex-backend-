...
export const getDashboard = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const totalSocialEarningsPool = await PostAnalytics.aggregate([
...
export const getUserFullDetails = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getCourseDetails = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const course = await Course.findById(req.params.id).populate('instructorId', 'firstName lastName email phone bankAccount');
    if (!course) {
      return res.status(404).json({ success: false, message: 'Course not found' });
    }

    const enrollments = await Enrollment.find({ courseId: course._id }).populate('userId', 'firstName lastName email');
    const transactions = await Transaction.find({ type: 'course_purchase', 'metadata.courseId': course._id }).sort(
      '-createdAt'
    );
...
export const approveBook = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await sendNotification({
      userId: book.uploadedBy.toString(),
      title: '📚 Book Approved',
      message: `Your book "${book.title}" has been approved and is now available in the library!`,
      type: 'system',
      channels: ['email', 'push'],
    });

    getIO().to(`user:${book.uploadedBy}`).emit('notification', {
      title: 'Book Approved',
      message: `Your book "${book.title}" is now live!`,
    });
...
export const rejectBook = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const book = await Book.findByIdAndUpdate(
      id,
      { approvalStatus: 'rejected', rejectionReason, isPublished: false },
      { new: true }
    );
    if (!book) {
      return res.status(404).json({ success: false, message: 'Book not found' });
    ...
    await sendNotification({
      userId: book.uploadedBy.toString(),
      title: '📚 Book Rejected',
      message: `Your book "${book.title}" was not approved. Reason: ${rejectionReason || 'Not specified'}.`,
      type: 'system',
      channels: ['email', 'push'],
    });
...
export const createAnnouncement = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { title, message, sendEmail = true, sendSms = false, sendPush = true } = req.body;
    ...
    const users = await User.find({}, '_id email phone pushSubscription');
...
export const getPublicAnnouncements = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const updateCoupon = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const coupon = await AdminCoupon.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!coupon) {
...
// ==================== MANUAL PAYMENTS ====================
export const getPendingManualPayments = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getAllManualPayments = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getManualPaymentStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const stats = {
      pending: await ManualPayment.countDocuments({ status: 'pending_review' }),
      approved: await ManualPayment.countDocuments({ status: 'approved' }),
...
export const getManualPaymentById = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const approveManualPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const rejectManualPayment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { rejectionReason } = req.body;
    const admin = req.user as any;
...
export const getChallengeParticipants = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { challengeId } = req.params;
    const progress = await ChallengeProgress.find({ challengeId }).populate('userId', 'firstName lastName email');
...
export const completeChallengeForUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getAllChallengeProgressStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// ==================== SOCIAL EARNINGS ====================
export const getSocialEarningsConfig = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const updateSocialEarningsConfig = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const admin = req.user as IUser;
...
export const getTopEarningPosts = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getTotalSocialEarningsPool = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const total = await PostAnalytics.aggregate([{ $group: { _id: null, total: { $sum: '$earnings' } } }]);
...
export const triggerSocialEarnings = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const analytics = await PostAnalytics.find({ totalEngagement: { $gt: 0 } }).populate('postId', 'authorId');
...
export const getPlatformStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const totalBooks = await Book.countDocuments({ isPublished: true, approvalStatus: 'approved' });
...
// ==================== ADMIN DELETE POST ====================
export const deletePostByAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
