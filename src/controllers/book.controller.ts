// ============================================================
// FILE: src/controllers/book.controller.ts (FIXED – Admin auto-approve)
// ============================================================
...
// ─── USER SUBMIT BOOK FOR APPROVAL (Premium users only) ──────────────
export const submitBookForApproval = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = req.user as IUser;

    // Check if user is premium (or admin, but admin should use admin create)
    ...
    const book = await Book.create({
      ...
      fileUrl,
      uploadedBy: user._id,
      isPublished: false,          // ❌ Not published until admin approves
...
export const createBook = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const book = await Book.create({
      ...
      uploadedBy: user._id,
      // ─── AUTO-APPROVED ────────────────────────────────────────────
      isPublished: true,              // ✅ Always published immediately
      status: 'approved',             // ✅ Always approved – no pending
      affiliatePercent: affiliatePercent || 0,
    });

    // Notify all admins (and maybe the user who uploaded – which is admin)
    const admins = await User.find({ roles: 'admin' }).select('_id');
    for (const admin of admins) {
...
export const listAllBooks = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const stats = {
      ...
      pending: await Book.countDocuments({ status: 'pending' }),
      approved: await Book.countDocuments({ status: 'approved' }),
      rejected: await Book.countDocuments({ status: 'rejected' }),
    };
...
export const listBooks = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const books = await Book.find({ isPublished: true, status: 'approved' })
      .sort('-createdAt')
      .skip(skip)
      .limit(Number(limit));

    const total = await Book.countDocuments({ isPublished: true, status: 'approved' });
...
// ─── PUBLIC: GET SINGLE BOOK (with purchase status) ────────────────
...
export const downloadBook = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Check if user has purchased the book (unless free)
    if (book.price > 0) {
